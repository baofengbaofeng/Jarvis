# Functional Test Suite 1.0.0-Preview — Design

**Date:** 2026-08-06  
**Status:** Approved  
**Scope:** Root `test/1.0.0-Preview` Electron + IPC functional regression for JARVIS desktop  
...
| Version directory | `test/1.0.0-Preview/` — uppercase `V`; new releases get new sibling dirs (e.g. `V1.1`); do not rewrite history |
| Automation depth | Electron + IPC; model-dependent paths use mock Provider |
| Layout vs existing e2e | Independent Playwright project under `test/1.0.0-Preview`; keep `apps/desktop/e2e` as smoke / CI lightweight suite |
| Package entry | Root `pnpm test:functional` → runs `test/1.0.0-Preview` |
...
Optional: `test/1.0.0-Preview/package.json` only if needed for local deps; prefer root/`apps/desktop` Playwright dependency and a root script that sets `cwd` to `test/1.0.0-Preview`.
...
- Global shortcuts / system overlay (product excluded from 1.0.0-Preview)
...
| `test/1.0.0-Preview` | Full functional regression for release 1.0.0-Preview |

Share patterns/helpers by copying or extracting shared helper modules later if duplication hurts; **do not** require `test/1.0.0-Preview` specs to import from `apps/desktop/e2e/*.spec.ts`. Existing smoke specs are not deleted or moved in this work.

## Acceptance criteria

1. `pnpm test:functional` runs the 1.0.0-Preview suite without a real Provider API key.
2. P0 + P1 pass on a clean machine after build/rebuild prerequisites.
3. `test/1.0.0-Preview/README.md` lists every suite file with tier (P0/P1/P2).
...
- [ ] `test/README.md` + `test/1.0.0-Preview/README.md`
