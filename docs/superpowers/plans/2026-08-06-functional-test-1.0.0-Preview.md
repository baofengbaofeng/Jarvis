# Functional Test Suite 1.0.0-Preview Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a runnable Electron+IPC functional regression suite under `test/1.0.0-Preview/` with mock Provider, thin README indexes, and root `pnpm test:functional`.

**Architecture:** Independent Playwright project at repo root `test/1.0.0-Preview` launches the real Electron main/preload against a Vite renderer (same pattern as `apps/desktop/e2e`), isolates each run with `JARVIS_DATA_DIR` + unique daemon port, and drives model-dependent paths through a local OpenAI-compatible mock HTTP server. Existing `apps/desktop/e2e` stays untouched.

**Tech Stack:** Playwright (`@playwright/test` from `apps/desktop`), Electron, Node `http` mock, TypeScript ESM, pnpm.

**Spec:** `docs/superpowers/specs/2026-08-06-functional-test-1.0.0-Preview-design.md`

## Global Constraints

- Version folder is `test/1.0.0-Preview/` — uppercase `V`; never `1.0.0-Preview`.
...
### Task 1: Scaffold `test/1.0.0-Preview` + root script + Playwright config

**Files:**
- Create: `test/README.md`
- Create: `test/1.0.0-Preview/README.md`
- Create: `test/1.0.0-Preview/playwright.config.ts`
- Create: `test/1.0.0-Preview/global-setup.ts`
- Modify: `package.json` (root) — add `"test:functional"` script
- Create: `test/1.0.0-Preview/suites/.gitkeep` (placeholder until Task 3)

**Interfaces:**
- Consumes: none
- Produces:
  - Root script: `pnpm test:functional` → `pnpm --dir apps/desktop exec playwright test --config ../../test/1.0.0-Preview/playwright.config.ts` (or equivalent cwd/`config` path that resolves `@playwright/test` from desktop)
...
- [ ] **Step 1: Write root README and 1.0.0-Preview README**
...
Versioned Electron + IPC suites. Directory names use uppercase `V` (e.g. `1.0.0-Preview`, `V1.1`).
...
Copy `1.0.0-Preview/` to `V1.1/`, update coverage matrix, point `test:functional` (or add `test:functional:V1.1`) at the new config.
```

`test/1.0.0-Preview/README.md` — coverage matrix table listing all 12 suite files with columns: Suite | Domains | Tiers (P0/P1/P2). Mark suites not yet implemented as “pending” until later tasks land.

- [ ] **Step 2: Write `global-setup.ts`**

Mirror `apps/desktop/e2e/global-setup.ts`: if `apps/desktop/out/main/index.js` missing or `CI` set, run `pnpm exec electron-vite build` in `apps/desktop`; always `pnpm exec electron-rebuild -f -w better-sqlite3` in `apps/desktop`. Resolve paths with `fileURLToPath` relative to `test/1.0.0-Preview` → `../../apps/desktop`.
...
```json
"test:functional": "pnpm --dir apps/desktop exec playwright test --config ../../test/1.0.0-Preview/playwright.config.ts"
```

Verify the relative `--config` path resolves when cwd is `apps/desktop` (Playwright resolves config path relative to cwd). If broken, use absolute path via `node -e` wrapper or `playwright test -c` from a small `test/1.0.0-Preview/run.mjs` that `chdir`s correctly — pick the first approach that works.
...
**Files:**
- Create: `test/1.0.0-Preview/helpers/electron-app.ts`
- Create: `test/1.0.0-Preview/helpers/mock-provider.ts`
- Create: `test/1.0.0-Preview/helpers/fixtures.ts`
- Create: `test/1.0.0-Preview/suites/00-helpers-smoke.spec.ts` (minimal: launch app, assert bridge, start/stop mock)
...
**Files:**
- Create: `test/1.0.0-Preview/suites/01-shell-onboarding.spec.ts`
- Create: `test/1.0.0-Preview/suites/02-providers-models.spec.ts`
- Create: `test/1.0.0-Preview/suites/03-agents-templates.spec.ts`
- Modify: `test/1.0.0-Preview/README.md` — mark 01–03 implemented
- Delete: `test/1.0.0-Preview/suites/.gitkeep` if present
...
**Files:**
- Create: `test/1.0.0-Preview/suites/04-chat-sessions.spec.ts`
- Create: `test/1.0.0-Preview/suites/05-task-board-approvals.spec.ts`
...
P2 Multica: `test.skip(true, 'Multica harness not in 1.0.0-Preview suite')`.
...
**Files:**
- Create: `test/1.0.0-Preview/suites/06-settings-mcp-skills.spec.ts`
- Create: `test/1.0.0-Preview/suites/07-coding-diff.spec.ts`
...
**Files:**
- Create: `test/1.0.0-Preview/suites/08-office-hub.spec.ts`
- Create: `test/1.0.0-Preview/suites/09-squad-workflow.spec.ts`
...
**Files:**
- Create: `test/1.0.0-Preview/suites/10-daemon-runtime.spec.ts`
- Create: `test/1.0.0-Preview/suites/11-data-safety-config.spec.ts`
- Create: `test/1.0.0-Preview/suites/12-shortcuts-usage-audit.spec.ts`
- Modify: `test/1.0.0-Preview/README.md` — full matrix current
...
**Files:**
- Modify: `test/README.md`, `test/1.0.0-Preview/README.md` as needed
...
- [ ] **Step 3: Self-review — grep `expect(true)` and `test.skip` under `test/1.0.0-Preview`; remove fake passes.
...
|------------------|------|
| `test/1.0.0-Preview/` uppercase V | 1 |
