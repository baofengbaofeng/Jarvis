...
- **E12 外部 IDE:** HTTP localhost(17891)暴露 `/open?file=`、`/diff?task=`;CLI `jarvis open --file path:line` 调 `code -g`;VS Code 插件后置 1.0.0-Preview。
...
**M4 已知后置:** 完整 Inline 补全(E5,已排除 Q3:B);Monaco 完整 IDE(K3 降级);sqlc 生成 daemon 侧 tasks 写入(M7);代码索引向量库切 sqlite-vec/LanceDB(可选,embedding 注入已预留);VS Code 插件(可选,后置 1.0.0-Preview)。
