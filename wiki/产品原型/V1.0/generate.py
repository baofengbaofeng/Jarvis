#!/usr/bin/env python3
"""Generate JARVIS V1.0 product prototype SVG wireframes."""
...
def proto_agent_list(cx, cy, cw, ch, active="Agent"):
    ...
    cards = [
        ...
        ("审查 Agent", "Code Review、测试", "my-model-3", "V1.0"),
        ("Squad Leader", "任务分派与协调", "my-model-1", "V1.0"),
    ]
...
def proto_code_panel(cx, cy, cw, ch, active="代码"):
    p = []
    p.append(header_bar(cx, cy, cw, "Diff 预览", "轻量代码面板 · K3/E9", ["V1.0", "E9"]))
    tree_w = 200
...
def proto_task_board(cx, cy, cw, ch, active="Task"):
    p = []
    p.append(header_bar(cx, cy, cw, "Task 看板", "任务生命周期 · K4/L4-L6", ["V1.0"]))
...
def proto_squad(cx, cy, cw, ch, active="Agent"):
    p = []
    p.append(header_bar(cx, cy, cw, "Squad 小队", "Leader + 成员 · Multica 对齐 Q1:A · F8/F9", ["V1.0"]))
...
def proto_settings(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "设置", "偏好与配置 · C1-C12", ["MVP/V1.0"]))
...
def proto_skills(cx, cy, cw, ch, active="Skills"):
    p = []
    p.append(header_bar(cx, cy, cw, "Skills 管理", "SKILL.md + 绑定 Agent · G1-G3/C4", ["MVP/V1.0"]))
...
def proto_mcp(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "MCP Server 管理", "stdio/SSE/HTTP · G4-G8/C3", ["MVP/V1.0"]))
    servers = [
...
def proto_multica_runtime(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "Multica Runtime", "仅 Runtime 被调度 · Q2:A · H1/L35-L39", ["V1.0"]))
    info = [
...
def proto_office_writing(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "AI 写作助手", "邮件/报告/文案 · D5", ["V1.0"]))
...
def proto_pdf_reader(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "PDF 伴读", "中英对照 · 摘要 · D7", ["V1.0"]))
...
def proto_webview(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "内置浏览器 · 网页总结", "WebView I8 + D8", ["V1.0"]))
...
def proto_approval(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "敏感操作确认", "人类-in-the-loop · J2/F15", ["MVP/V1.0"]))
...
def proto_canvas(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "Canvas 可视化工作区", "结构化输出展示 · K6", ["V1.0"]))
...
def proto_search(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "全局搜索", "对话/Agent/Task · L21", ["V1.0"]))
...
def proto_split_view(cx, cy, cw, ch, active="代码"):
    p = []
    p.append(header_bar(cx, cy, cw, "分屏视图", "聊天 + 文件预览 · K7", ["V1.0"]))
...
def proto_plan_mode(cx, cy, cw, ch, active="代码"):
    p = []
    p.append(header_bar(cx, cy, cw, "Plan 模式", "只读分析，不改文件 · E10", ["V1.0"]))
...
def proto_task_log(cx, cy, cw, ch, active="Task"):
    p = []
    p.append(header_bar(cx, cy, cw, "Task 执行日志", "流式双通道 · K5/L5", ["V1.0"]))
    logs = [
...
def proto_selection_menu(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "划词菜单", "App 内划词 · D4", ["V1.0"]))
...
def proto_voice(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "语音输入", "App 内语音对话 · D11", ["V1.0"]))
    ...
    p.append(text(cx + cw / 2, cy + 370, "（TTS 播报 L24 不在 V1.0 范围）", 12, COLORS["muted"], "normal", "middle"))
    return "".join(p)
...
def proto_translate(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "边写边译", "实时翻译辅助 · D6", ["V1.0"]))
...
def proto_video_summary(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "视频内容摘要", "链接/上传 · D9", ["V1.0"]))
...
def proto_image_gen(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "文生图", "图像生成 · D10", ["V1.0"]))
...
def proto_file_upload(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "文件上传分析", "PDF/Word/Excel/PPT · D12", ["V1.0"]))
...
def proto_prompt_templates(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "Prompt 模板库", "可复用提示词 · D15", ["V1.0"]))
...
def proto_token_stats(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "Token 用量统计", "成本估算 · B9", ["V1.0"]))
...
def proto_shortcuts(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "快捷键配置", "App 内快捷键 · C5", ["V1.0"]))
    ...
    for action, key in shortcuts:
        ...
        y += 52
    p.append(text(cx + 40, cy + ch - 50, "注: 全局快捷键唤醒 A3 不在 V1.0 范围", 12, COLORS["muted"]))
    return "".join(p)


def proto_permissions(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "工具权限 / 沙箱", "allow/deny 策略 · C6/J3/J6", ["V1.0"]))
...
def proto_config_io(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "配置导入 / 导出", "JSON/YAML · C12", ["V1.0"]))
...
def proto_daemon(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "Daemon 管理", "状态/重启/资源 · L7-L9", ["V1.0"]))
    metrics = [("Daemon", "运行中 ●", "重启"), ("CPU", "12%", ""), ("内存", "486 MB", ""), ("并发 Task", "2 / 6", ""), ("jarvis-agent", "v1.0.0", "—health")]
    y = cy + 80
...
def proto_mcp_approval(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "MCP 权限审批", "首次调用确认 · J7/G8", ["V1.0"]))
...
def proto_agent_templates(cx, cy, cw, ch, active="Agent"):
    p = []
    p.append(header_bar(cx, cy, cw, "Agent 模板库", "预设模板 · L30", ["V1.0"]))
...
def proto_backup(cx, cy, cw, ch, active="设置"):
    p = []
    p.append(header_bar(cx, cy, cw, "数据备份与恢复", "SQLite 备份/迁移 · L18-L20", ["V1.0"]))
...
def proto_at_reference(cx, cy, cw, ch, active="代码"):
    p = []
    p.append(header_bar(cx, cy, cw, "@ 引用", "文件/文件夹/符号 · E6", ["V1.0"]))
...
def proto_runtime_indicator(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "运行模式指示", "本地 vs Runtime · L39", ["V1.0"]))
    ...
    p.append(text(cx + 40, cy + 240, "注: Multica Client H2 不在 V1.0 范围", 12, COLORS["muted"]))
    return "".join(p)


def proto_image_understand(cx, cy, cw, ch, active="对话"):
    p = []
    p.append(header_bar(cx, cy, cw, "多模态图片理解", "截图/上传识图 · L23", ["V1.0"]))
...
PROTOTYPES = [
    ...
    ("07-轻量代码面板-Diff", "K3/E9", "V1.0", proto_code_panel, "代码"),
    ("08-Task看板", "K4/L4", "V1.0", proto_task_board, "Task"),
    ("09-Squad小队协作", "F8/F9", "V1.0", proto_squad, "Agent"),
    ("10-设置页", "C1-C12", "MVP/V1.0", proto_settings, "设置"),
    ("11-Skills管理", "G1/C4", "MVP/V1.0", proto_skills, "Skills"),
    ("12-MCP管理", "G4/C3", "MVP/V1.0", proto_mcp, "设置"),
    ("13-Multica-Runtime状态", "H1/L35", "V1.0", proto_multica_runtime, "设置"),
    ("14-AI写作助手", "D5", "V1.0", proto_office_writing, "对话"),
    ("15-PDF伴读", "D7", "V1.0", proto_pdf_reader, "对话"),
    ("16-内置浏览器-网页总结", "I8/D8", "V1.0", proto_webview, "对话"),
    ("17-窗口吸附模式", "A4", "MVP", proto_dock_mode, "对话"),
    ("18-系统托盘菜单", "A2/L7", "MVP", "V1.0", None),
    ("19-敏感操作确认", "J2/F15", "MVP/V1.0", proto_approval, "对话"),
    ("20-Canvas可视化", "K6", "V1.0", proto_canvas, "对话"),
    ("21-全局搜索", "L21", "V1.0", proto_search, "对话"),
    ("22-分屏-聊天加文件", "K7", "V1.0", proto_split_view, "代码"),
    ("23-Plan模式", "E10", "V1.0", proto_plan_mode, "代码"),
    ("24-Task执行日志", "K5/L5", "V1.0", proto_task_log, "Task"),
    ("25-划词菜单", "D4", "V1.0", proto_selection_menu, "对话"),
    ("26-语音输入", "D11", "V1.0", proto_voice, "对话"),
    ...
    ("30-边写边译", "D6", "V1.0", proto_translate, "对话"),
    ("31-视频内容摘要", "D9", "V1.0", proto_video_summary, "对话"),
    ("32-文生图", "D10", "V1.0", proto_image_gen, "对话"),
    ("33-文件上传分析", "D12/L22", "V1.0", proto_file_upload, "对话"),
    ("34-Prompt模板库", "D15", "V1.0", proto_prompt_templates, "对话"),
    ("35-对话导出", "D14", "MVP", proto_export, "对话"),
    ("36-Token用量统计", "B9", "V1.0", proto_token_stats, "设置"),
    ("37-快捷键配置", "C5", "V1.0", proto_shortcuts, "设置"),
    ("38-权限沙箱配置", "C6/J3/J6", "V1.0", proto_permissions, "设置"),
    ("39-工作区绑定", "C7/L10", "MVP", proto_workspace, "设置"),
    ("40-日志调试面板", "C11", "MVP", proto_debug_log, "设置"),
    ("41-配置导入导出", "C12", "V1.0", proto_config_io, "设置"),
    ("42-Daemon管理", "L7-L9", "V1.0", proto_daemon, "设置"),
    ("43-MCP权限审批", "J7/G8", "V1.0", proto_mcp_approval, "对话"),
    ("44-Agent模板库", "L30", "V1.0", proto_agent_templates, "Agent"),
    ("45-数据备份恢复", "L18-L20", "V1.0", proto_backup, "设置"),
    ("46-Task取消重试", "L4-L6", "MVP", proto_task_control, "Task"),
    ("47-At引用文件", "E6", "V1.0", proto_at_reference, "代码"),
    ("48-运行模式指示", "L39/A8", "V1.0", proto_runtime_indicator, "对话"),
    ("49-多模态图片理解", "L23", "V1.0", proto_image_understand, "对话"),
]

# fix entry 18 typo
PROTOTYPES[17] = ("18-系统托盘菜单", "A2/L7", "MVP/V1.0", proto_tray, "对话")
  ...
  <div class="app">
    <aside class="sidebar">
      <div class="sidebar-header">
        <h1>JARVIS 产品原型</h1>
        <p>V1.0 · 共 {len(manifest)} 个界面 · 2026-08-02</p>
      </div>
