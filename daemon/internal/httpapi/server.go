...
func NewServer(version string, q *runtime.Queue, extras ...ServerExtra) *Server {
	return NewServerWithAuth(version, q, "", extras...)
}

func NewServerWithAuth(version string, q *runtime.Queue, token string, extras ...ServerExtra) *Server {
	s := &Server{version: version, queue: q, mux: http.NewServeMux(), token: token}
...
func (s *Server) Handler() http.Handler { return authMiddleware(s.token, s.mux) }
